...
app.post("/enquiry", (req, res) => {
  var emp = req.body;
  var sql =
    "insert into enquiry_table(enquiry_name,enquiry_mobileno,enquiry_message) values(?,?,?);";
  mysqlconnection.query(
    sql,
    [emp.enquiry_name, emp.enquiry_mobileno, emp.enquiry_message],
    (err, rows, fields) => {
      if (!err) {
        res.send("success");
      } else {
...
app.get("/feedback/:id", (req, res) => {
  mysqlconnection.query(
    "SELECT * FROM feedback_table WHERE feedback_id = ?",
    [req.params.id],
    (err, rows, fields) => {
      if (!err)
        // console.log(row)
...
app.delete("/feedback/:id", (req, res) => {
  mysqlconnection.query(
    "DELETE FROM feedback_table WHERE feedback_id = ?",
    [req.params.id],
    (err, rows, fields) => {
      if (!err)
        // console.log(row)
...
app.post("/feedback-insert", (req, res) => {
  ...
  var sql =
    "INSERT INTO feedback_table (feedback_name, feedback_emaill, feedback_message) VALUES (?,?,?);";

  mysqlconnection.query(
    sql,
    [emp.feedback_name, emp.feedback_emaill, emp.feedback_message],
    (err, rows, fields) => {
      if (!err) {
        console.log(rows);
        // // console.log(row)
...
app.get("/bloodbank/:username/:password", (req, res) => {
  var sql =
    "SELECT bloodbank_username,bloodbank_password FROM bloodbank_table where bloodbank_username=? and bloodbank_password=?;";
  mysqlconnection.query(
    sql,
...
app.post("/bloodbank", (req, res) => {
  var emp = req.body;
  var sql =
    "insert into bloodbank_table(bloodbank_name,bloodbank_location,bloodbank_contactno,bloodbank_username,bloodbank_password) values(?,?,?,?,?);";
  mysqlconnection.query(
    sql,
    [
      emp.bloodbank_name,
      emp.bloodbank_location,
      emp.bloodbank_contactno,
      emp.bloodbank_username,
      emp.bloodbank_password,
    ],
    (err, rows, fields) => {
      if (!err) {
        res.send("success");
...
app.get("/verify_user/:mobileno", (req, res) => {
  ...
  mysqlconnection.query(sql, [req.params.mobileno], (err, rows, fields) => {
    ...
    if (b == "0") {
      ...
      (message =
        "Namasthe, Get your running shoes ready to meet the fellow runners stepping their foot for cause. Just few hours to go. Here are your route maps THIS IS TEST MAP LOCATION ONLY -Seva Bharathi"),
  ...
  mysqlconnection.query(sql, [req.params.mobileno], (err, rows, fields) => {
    ...
    if (b == "0") {
      ...
      var error_json = {
        ...
        status: "fail",
        message:
          "mobile number already exist (You are already registered goto login page then login)",
      };
      res.send(error_json);
    }
  });
...
app.get("/state/:cid", (req, res) => {
  mysqlconnection.query(
    "select * from state_table where country_id=?;",
    [req.params.cid],
    (err, rows, fields) => {
      if (!err) {
        var response = {
...
app.get("/district/:sid", (req, res) => {
  mysqlconnection.query(
    "select * from district_table where state_id=?;",
    [req.params.sid],
    (err, rows, fields) => {
      if (!err) {
        var response = {
...
app.get("/area/:did", (req, res) => {
  mysqlconnection.query(
    "select * from area_table where district_id=?;",
    [req.params.did],
    (err, rows, fields) => {
      if (!err) {
        var response = {
...
app.get("/benf", (req, res) => {
  mysqlconnection.query("select * from benf_table", (err, rows, fields) => {
    if (!err) {
      var response = {
        data: rows,
        status: "success",
...
app.get("/benf/:mobile1", (req, res) => {
  var sql = "SELECT * from benf_table where benf_mobile1=?;";
...
app.post("/benfsignup", (req, res) => {
  var emp = req.body;
  var sql =
    "INSERT INTO benf_table(benf_fullname, benf_age,benf_gender,benf_blood_type,benf_bloodgroup,benf_units_required,benf_acc_bloodgroup,benf_deadline,benf_mobile1,benf_mobile2,benf_hospital_name,benf_hospital_address,benf_country,benf_state,benf_district,benf_area,benf_pincode,benf_status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1);";
  mysqlconnection.query(
    sql,
    [
      emp.benf_fullname,
      ...
      emp.benf_blood_type,
      emp.benf_bloodgroup,
      emp.benf_units_required,
      emp.benf_acc_bloodgroup,
      emp.benf_deadline,
      emp.benf_mobile1,
      emp.benf_mobile2,
      emp.benf_hospital_name,
      emp.benf_hospital_address,
      emp.benf_country,
      emp.benf_state,
      emp.benf_district,
      emp.benf_area,
...
app.get("/accbloodgroup", (req, res) => {
  mysqlconnection.query(
    "select * from accepted_bloodgroup_table;",
    (err, rows, fields) => {
      if (!err) {
        var response_json = {
          data: rows,
...
//user donor sign up form
app.get("/user", (req, res) => {
  mysqlconnection.query("select * from user_table;", (err, rows, fields) => {
    if (!err) {
      var response_json = {
        data: rows,
        status: "success",
...
//MANAGE DONARS GETTING THE INFORMATION
...
app.get("/user/:mobile1", (req, res) => {
  mysqlconnection.query(
    "select * from user_table where user_mobile1=?;",
    [req.params.mobile1],
    (err, rows, fields) => {
      if (!err) {
        var response_json = {
...
app.delete("user/:mobile1", (req, res) => {
  mysqlconnection.query(
    "delete from user_table where user_mobile1=?",
    [req.params.mobile1],
    (err, rows, fields) => {
      if (!err) {
        var response_json = {
...
app.post("/usersignup", (req, res) => {
  var emp = req.body;
  var sql =
    "INSERT INTO user_table(user_fullname,user_age,user_gender,user_bloodgroup,user_mobile1,user_mobile2,user_emailid,user_fulladdress,user_country,user_state,user_district,user_area,user_pincode,userstatus) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
  mysqlconnection.query(
    sql,
    [
      emp.user_fullname,
...
app.get("/data", (req1, res1) => {
  mysqlconnection.query(
    "SELECT * FROM blooddonation.benf_table where benf_status=1",
    (err, rows, fields1) => {
      ...
      function add(msg) {
        ...
        var sql =
          "select * from user_table where user_table.userstatus=1 and user_table.user_bloodgroup=" +
          msgblood +
          ";";
        mysqlconnection.query(sql, (err, rows, fields) => {
          if (!err) {
            for (var i = 0; i < rows.length; i++) {
              ...
              url =
                "https://e62c99bb016cfef646886992c7f1002e72223e8ec6a68661:cc88955507b18eb154b0f8051683fc9c8d4c1ad9005d11a5@api.exotel.com/v1/Accounts/sevabharathi1/Calls/connect.json";
              axios
                .post(
                  url,
                  r({
